...
def get_users():
  try:
    users = execute_query("SELECT * FROM Users", fetchone=False, fetchall=True)
...
def add_user():
  try:
    ...
    existing_user = execute_query("SELECT * FROM Users WHERE username = %s OR email = %s", (username, email))
    if existing_user:
